 ...
 private:
  void timerCallback() {
    try {
      ...
      transform_stamped =
          tf_buffer_.lookupTransform("panda_link0", "panda_link8", tf2::TimePointZero);
      ...
      deviation -= tf2::Vector3(0.32, 0.0, 0.6);
      deviation *= -0.1;
      ...
      ee_velocity_msg.header.frame_id = "panda_link0";
  ...
  double calculateVelocity(double y) {
    double max_velocity = 0.8;  // Maximum velocity
    double acceleration = 2.0;  // Acceleration magnitude
